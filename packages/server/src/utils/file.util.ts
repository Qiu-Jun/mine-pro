 ...
 * @Author: June
 * @Description: 
 * @Date: 2024-06-14 15:31:57
 * @LastEditTime: 2024-06-14 16:49:51
 * @LastEditors: June
 * @FilePath: \mine-pro\packages\server\src\utils\file.util.ts
 */
...
export async function saveLocalFile(buffer: Buffer, name: string, currentDate: string, type: string) {
  const filePath = path.join(__dirname, '../../', 'uploads/upload/', `${currentDate}/`, `${type}/`)
  try {
    // 判断是否有该文件夹
...
export async function saveFile(file: MultipartFile, name: string) {
  const filePath = path.join(__dirname, '../../', 'uploads/upload', name)
