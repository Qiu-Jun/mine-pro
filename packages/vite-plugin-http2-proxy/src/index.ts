...
export default (options?: Record<string, ProxyOptions>): Plugin => {
  ...
  if (version.startsWith('v2')) {
    console.warn(
      '[@minepro/vite-plugin-http2-proxy] http2-proxy is not supported in Node.js v20.x+'
    )
    return { name: '@minepro/vite-plugin-http2-proxy' }
  }
  ...
  return {
    name: '@minepro/vite-plugin-http2-proxy',
    config: (config) => {
      const { server } = config
